...
# from LassyExtraction.utils.PostProcess import get_unique, freqsort, count_occurrences
import pickle
...
class MyHackyExample(object):
    def __init__(self, word=None, enc=None, idxs=None, padding=None, hidden_dim=None):
        self.enc = enc
        self.word = word
        self.idxs = idxs
    ...
    @property
    def shape(self):
        return (*self.word.shape, self._hidden_dim)
...
def do_everything_ccg(args, hidden_dim):
    from ccg.util.loader import load_supertags
    from ccg.parser.scoring.supertag import build_topdown_supertagger

    st = build_topdown_supertagger(args, training_files=[1])

    trainloader = list(load_supertags(args.training_files, st, format=args.training_format,
                                      load_phrasal=False, load_deps=False,
                                      batch_size=args.batch_size, train=True, lower=True, output_sentences=False))

    devloader = list(load_supertags(args.development_files, st, format=args.development_format,
                                      load_phrasal=False, load_deps=False,
                                      batch_size=args.batch_size, train=False, lower=True, output_sentences=False))

    testloader = list(load_supertags(args.testing_files, st, format=args.testing_format,
                                      load_phrasal=False, load_deps=False,
                                      batch_size=args.batch_size, train=False, lower=True, output_sentences=False))
    ...
    for data in trainloader:
        X.append(MyHackyExample(hidden_dim=hidden_dim, **data['x']))
        Y.append(data['y'][0])
